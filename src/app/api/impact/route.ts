    ...
    const sql = `
      WITH viral_orders AS (
        ...
        SELECT vo.APPLICATION_USER_ID,
               ...
               MAX(CASE WHEN oh.CREATED_AT::DATE < TO_DATE('${safeDate}') THEN oh.CREATED_AT END)::DATE AS LAST_TURBO_BEFORE
    ...
    try {
      const productUserSql = `
        ...
        prev_product AS (
          ...
          JOIN RP_SILVER_DB_PROD.DES_PROD.ORDERS o_prev ON o_prev.APPLICATION_USER_ID = vb.APPLICATION_USER_ID AND o_prev.COUNTRY = 'MX' AND o_prev.CREATED_AT::DATE < TO_DATE('${safeDate}')
