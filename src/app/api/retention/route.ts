    ...
    const sql = `
      ...
      turbo_history AS (
        SELECT vu.APPLICATION_USER_ID, MIN(o.CREATED_AT)::DATE AS FIRST_TURBO,
          MAX(CASE WHEN o.CREATED_AT::DATE < TO_DATE('${safeDate}') THEN o.CREATED_AT END)::DATE AS LAST_TURBO_BEFORE
        FROM viral_users vu
        LEFT JOIN RP_SILVER_DB_PROD.DES_PROD.ORDERS o ON o.APPLICATION_USER_ID = vu.APPLICATION_USER_ID AND o.COUNTRY = 'MX' AND o.STORE_TYPE_STORE LIKE '%turbo%'
        GROUP BY 1
      ),
      classified AS (
      ...
      post_orders AS (
        SELECT c.APPLICATION_USER_ID, c.USER_TYPE,
          MAX(CASE WHEN o2.CREATED_AT::DATE BETWEEN DATEADD(day,1,TO_DATE('${safeDate}')) AND DATEADD(day,15,TO_DATE('${safeDate}')) THEN 1 ELSE 0 END) AS HAD_ORDER_15D,
          MAX(CASE WHEN o2.CREATED_AT::DATE BETWEEN DATEADD(day,1,TO_DATE('${safeDate}')) AND DATEADD(day,30,TO_DATE('${safeDate}')) THEN 1 ELSE 0 END) AS HAD_ORDER_30D,
          MAX(CASE WHEN o2.CREATED_AT::DATE BETWEEN DATEADD(day,1,TO_DATE('${safeDate}')) AND DATEADD(day,45,TO_DATE('${safeDate}')) THEN 1 ELSE 0 END) AS HAD_ORDER_45D,
          MAX(CASE WHEN o2.CREATED_AT::DATE BETWEEN DATEADD(day,1,TO_DATE('${safeDate}')) AND DATEADD(day,60,TO_DATE('${safeDate}')) THEN 1 ELSE 0 END) AS HAD_ORDER_60D,
          COUNT(DISTINCT o2.ORDER_ID) AS TOTAL_ORDERS_POST
        FROM classified c
        LEFT JOIN RP_SILVER_DB_PROD.DES_PROD.ORDERS o2 
          ON o2.APPLICATION_USER_ID = c.APPLICATION_USER_ID AND o2.COUNTRY = 'MX' AND o2.STORE_TYPE_STORE LIKE '%turbo%'
          AND o2.CREATED_AT::DATE BETWEEN DATEADD(day,1,TO_DATE('${safeDate}')) AND DATEADD(day,60,TO_DATE('${safeDate}'))
        GROUP BY 1, 2
      )
      SELECT USER_TYPE,
